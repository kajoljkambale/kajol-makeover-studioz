...
export const sb = supabase  // alias used in tab components
...
export async function dbUpsert(table, row) {
  ...
  if (error) { console.error('Upsert ' + table + ':', error); alert('Save error (' + table + '): ' + error.message); return null }
  return data?.[0]
}
...
export async function loadAllData() {
  const [students, courses, batches, classes, homeworkCompliance,
         payments, orders, expenses, enrollmentRequests] = await Promise.all([
    dbLoad('students'),
    dbLoad('courses'),
    dbLoad('batches'),
    dbLoad('classes'),
  ...
  return { students, courses, batches, classes, homeworkCompliance,
           payments, orders, expenses, enrollmentRequests }
}

export async function clearAllData() {
  const tables = ['homework_compliance', 'payments', 'expenses', 'orders',
